$(function() {
	...
	$('#newAnalysisCategory').click(function() {
		asyncbox.open({
			title: '新增环境要素',
			url: 'analysisCategory_analysisCategory_addPage.action',
			width: 700,
			height: 400,
			btnsbar: [{
				text: '保　存', 
			...
			callback: function(action,opt) {　　　　
				if (action == 'ok') {　　　　　　　 
					var oForm=opt.document.getElementById("analysisGategoryAdd");
					var $_oForm=$(oForm);
					...
					$.ajax({
						url : "analysisCategory_analysisCategory_add.action",
						data:$_oForm.serialize(),
						async : false,
						success : function(data) {
							var json = eval('(' + data + ')');
							if(json.info=='success'){
						    	asyncbox.alert('环境要素添加成功！', '通知',function(){
						    		window.location.reload();
						    	});
					    	}else{
					    		asyncbox.error('环境要素添加失败，请刷新后重新尝试！', '通知');
					    	}
							return false;
						}
					});
	...
	$('.changeAnalysisCatrgory').click(function() {
		var id=$(this).attr("data-num");
		asyncbox.open({
			title: '修改环境要素',
			url: 'analysisCategory_analysisCategory_changePage.action?viewId='+id,
			width: 700,
			height: 400,
			btnsbar: [{
				text: '修　改', // 按钮文本
			...
			callback: function(action, opt) {　　　　　 // 判断 action 值。
				if (action == 'change') {　　　　　　　 // 在回调函数中 this.id 可以得到该窗口
					...
					$.ajax({
						url : "analysisCategory_analysisCategory_change.action",
						data:$_oForm.serialize(),
						async : false,
						success : function(data) {
							var json = eval('(' + data + ')');
							if(json.info=='success'){
						    	asyncbox.alert('环境要素修改成功！', '通知',function(){
						    		window.location.reload();
						    	});
					    	}else{
					    		asyncbox.error('环境要素修改失败，请刷新后重新尝试！', '通知');
					    	}
							return false;
						}
					});
	...
	$('#delMoreAnalysisCategory').click(function() {
		var ids="";
		...
		if(ids==""){
			asyncbox.alert('请先选择需要删除的环境要素！', '删除结果');
			return false;
		};
		asyncbox.confirm('是否批量删除所有选中的环境要素？', '警告', function(action) {　
			if (action == 'ok') {
				$.ajax({
					url:"analysisCategory_analysisCategory_deleteAll.action",
					data:{'ids':ids},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
...
　　　			
	...
	$('.delAnalysisCategory').click(function() {
		var value=$(this).attr("data-id");
		var $_this=$(this);
		asyncbox.confirm('是否删除该环境要素信息？', '警告', function(action) {
			if (action == 'ok') {
				$.ajax({
					url:"analysisCategory_analysisCategory_delete.action",
					data:{'viewId':value},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
