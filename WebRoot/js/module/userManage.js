$(function() {
					...
					var options = {
							    ...
							    success : function(data,status){
							    	...
							    	if(json.info=='success'){
							    		asyncbox.alert('用户新增成功！', '新增结果',function(){
								    		window.location.reload();
								    	});
							    	}else{
							    		asyncbox.error('用户新增失败！', '新增结果',function(){
								    		window.location.reload();
								    	});
							    	}
							    },
	...
	$('.changeUser').click(function() {
		...
		asyncbox.open({
			title: '修改用户信息',
			url: 'userManage_user_changePage.action?id='+id,
			width: 700,
			height: 700,
			btnsbar: [{
				text: '修　改', // 按钮文本
	...
	$('.viewUser').click(function() {
		...
		asyncbox.open({
			title: '查看用户信息',
			url: 'userManage_user_viewPage.action?id='+id,
			width: 700,
			height: 600,
			btnsbar:$.btn.CANCEL,
		});
	...
	$('#delMoreUser').click(function() {
		...
		if(ids==""){
			asyncbox.alert('请先选择需要删除的用户数据！', '删除结果');
			return false;
		};
		asyncbox.confirm('是否批量删除所有选中的用户？删除之后该用户将无法登录该系统！', '警告', function(action) {　
			if (action == 'ok') {
				$.ajax({
					url:"userManage_user_deleteAll.action",
					data:{'ids':ids},
					async:false,
...
　　　			
	...
	$('.delUser').click(function() {
		...
		var $_this=$(this);
		asyncbox.confirm('是否删除该用户信息？删除之后将用户将无法登录该系统！', '警告', function(action) {
			if (action == 'ok') {
				$.ajax({
					url:"userManage_user_delete.action",
					data:{'id':value},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
	...
	$('#seachBtn').click(function(){
		...
		if(dataStart&&(!dataEnd)){
			asyncbox.alert('请输入入职结束时间！', '查询结果');
			return false;
		}
		if(dataEnd&&(!dataStart)){
			asyncbox.alert('请输入入职开始时间！', '查询结果');
			return false;
		}
		if($('#searchChoose').prop("value")=="choose"){
			 window.location.href="userManage_user_query.action?chooseBtn=1&&queryStr="+str;
			 return false;
		}
		 window.location.href="userManage_user_query.action?queryStr="+str;
	});
	$('#allResult').click(function(){
		window.location.href="userManage_user_main.action";
	});
	$('#allResultChoose').click(function(){
		window.location.href="userManage_user_choosePage.action";
	});
	$('.resetPWD').click(function(){
		var value=$(this).attr("data-num");
		var $_this=$(this);
		asyncbox.confirm('是否重置该用户密码？', '警告', function(action) {
			if (action == 'ok') {
				$.ajax({
					url:"userManage_user_resetPWD.action",
					data:{'id':value},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
	...
	$('#resetAllPWD').click(function(){
		...
		if(ids==""){
			asyncbox.alert('请先选择需要重置密码的用户数据！', '重置结果');
			return false;
		};
		asyncbox.confirm('是否批量重置所有选中的用户密码？', '警告', function(action) {　
			if (action == 'ok') {
				$.ajax({
					url:"userManage_user_resetAllPWD.action",
					data:{'ids':ids},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
