$(function() {
	...
	$('#exportList').on('click',function(){
		var ids="";
		$('#auditTable').find('tbody input[type="checkbox"]').each(function(){
			if($(this).prop('checked')){
				ids+=$(this).attr("data-search")+";";
			}
		});
		if(ids==""){
			asyncbox.alert('请先选择需要导出的记录！', '选择记录');
			return false;
		};
		asyncbox.confirm('是否根据选中的合同记录生成业务登记表？', '警告', function(action) {　
			if (action == 'ok') {
				window.location.href="contractManage_contractList_exportList.action?exportIds="+ids;
			}
		});
	});
	...
	$('.export').click(function(){
		...
		asyncbox.confirm('是否导出该合同？', '警告', function(action) {　
			if (action == 'ok') {
				window.location.href="contractManage_contract_exportTContract.action?viewId="+id;
			}
		});
	});
	...
	$('.into').click(function() {
		...
		asyncbox.confirm('是否将该合同进入流程？', '警告', function(action) {　
			...
			if (action == 'ok') {
				$.ajax({
					url : "contractManage_contract_flowInto.action?viewId=" + id,
					async : false,
					success : function(data) {
						...
						if (json.info == "failed") {
							asyncbox.error("创建项目失败，请刷新后重新尝试！", '流转结果');
						}
									...
									.confirm(
											...
											function(action) {
												if (action == 'ok') {
													$
															.ajax({
																url : "contractManage_contractList_deleteContract.action?contractType=0&&viewId="
																		+ id,
																async : false,
																success : function(
																		data) {
									...
									.confirm(
											...
											function(action) {
												if (action == 'ok') {
													$
															.ajax({
																url : "contractManage_contractList_stopContract.action?contractType=0&&viewId="
																		+ id,
																async : false,
																success : function(
																		data) {
