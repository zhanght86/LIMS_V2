$(function() {
	...
	$('.referAll').click(function(){
		...
		asyncbox.open({
			title : '项目审核页面',
			url : 'resultAuditSecond_resultAuditSecond_referPage.action?viewId=' + id,
			width : 850,
			height : 500,
			btnsbar: [{
				text: '审　核', 
			...
			callback : function(action, opt) {
				...
				if(action=="save"){
					...
					if(result==0){
						// 全部选“是”
						str="是否对该合同项目进行实验分析审核，您的审核意见为<b style='color:red;'>【审核通过】</b>，审核完成后您将无法修改审核结果，项目即将进入下一流程！";
					}else{
						// 选择了“否”
						str="是否对该合同项目进行实验分析审核，您的审核意见为<b style='color:red;'>【审核不通过】</b>，审核完成之后您将无法修改审核结果，项目即将返回实验分析流程！";
					}
					asyncbox.confirm(str, '业务流转', function(action) {
						if (action == 'ok') {
							$.ajax({
								url:"resultAuditSecond_resultAuditSecond_add.action?resultType="+result,
