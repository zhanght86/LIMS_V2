$(function() {
	...
	$('.referRecord').click(function(){
		...
		asyncbox.confirm('您正在对该项目的实验分析结果执行<b style="color:red;">【审核'+str+'】</b>操作，请确认您的选择？','警告', function(action) {　
			if (action == 'ok') {
				$.ajax({
					url:"resultItemAudit_resultItemAudit_refer.action",
					data:{'viewId':id,'type':type},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
						if(json.info=="errorFlow"){
							asyncbox.alert("您已成功对项目进行审核！由于尚有项目未通过审核，流程暂不进入一审阶段！", '结果',function(){
								window.location.reload();
							});						
						};
						if(json.info=="successFlow"){
							asyncbox.alert("您已成功对项目进行审核！由于所有项目均已复核通过，流程流转至一审阶段！", '结果',function(){
								window.location.reload();
							});						
						};
						if(json.info=="failed"){
	...
	$('.referAll').click(function(){
		...
		asyncbox.confirm('您正在对该项目下的所有实验分析结果执行<b style="color:red;">【审核'+str+'】</b>操作（注意：不会覆盖您单独指定的复核规则！），请确认您的选择？','警告', function(action) {　
			if (action == 'ok') {
				$.ajax({
					url:"resultItemAudit_resultItemAudit_referAll.action",
					data:{'viewId':id,'type':type},
					async:false,
					success:function(data){
						var json = eval('('+data+')');
						...
						if(json.info=="successFlow"){
							asyncbox.alert("您已成功对项目进行审核！由于所有项目均已审核通过，流程流转至一审阶段！", '结果',function(){
								window.location.reload();
							});						
						};
						if(json.info=="failed"){
	...
	$('.flow').click(function(){
		var id = $(this).attr("data-id");
		asyncbox.confirm('您即将流转该项目，请注意以下事项：<br/> 1.当前项目即将流转至"项目一审"阶段。<br/>2.您审核的不同送检单会被系统自动合并成一份送达一审人员手中。<br/>3.您可以在"项目列表"功能中查看已经流转的项目。','警告', function(action) {　
			if (action == 'ok') {
				$.ajax({
