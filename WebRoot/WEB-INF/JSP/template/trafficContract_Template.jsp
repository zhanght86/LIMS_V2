...
<%
	...
	doc.openDataRegion("PO_projectType").setValue(entity.getProjectType());
	...
	poCtrl.addCustomToolButton("-", "", 0);
	poCtrl.addCustomToolButton("流转", "flowWord", 13);
...
<script type="text/javascript">
	...
	function flowWord() {
		$.ajax({
			url : "contractManage_contract_flow.action?viewId="
					+ $('#viewId').val(),
			async : false,
			success : function(data) {
				var json = eval('(' + data + ')');
				if (json.info == "success") {
					//asyncbox.alert("创建项目成功，进入任务下达流程！", '流转结果');
				...
				if (json.info == "failed") {
					...
					document.getElementById("PageOfficeCtrl1").Alert(
							"创建项目失败，请到合同列表界面重新尝试进入流程！");
				}
				if (json.info == "container") {
					//asyncbox.alert("当前合同项目已经存在，无法重复创建！", '流转结果');
					document.getElementById("PageOfficeCtrl1").Alert(
							"当前合同项目已经存在，无法重复创建！");
				}
			}
