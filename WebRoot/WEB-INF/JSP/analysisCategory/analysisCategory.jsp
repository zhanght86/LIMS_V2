...
<script src="js/module/analysisCategory.js" type="text/javascript"
	charset="utf-8"></script>
...
<body>
	...
	<div class="formbody">
		...
		<div id="usual1" class="usual">
			...
			<div class="itab">
				<ul>
					<li><a href="#tab1" class="selected">环境要素管理</a></li>
				</ul>
			</div>
			<div id="tab1" class="tabson">
				<div class="tools">
					<ul class="toolbar">
						<li class="click" id="newAnalysisCategory"><span><img
								src="images/t01.png" /></span>新增</li>
						<li class="click" id="delMoreAnalysisCategory"><span><img
				...
				<table class="tablelist" id="analysisProjectTable">
					...
					<tbody>
						<s:iterator value="#analysisCategorieList" id="a">
							<tr>
								<td><input name="" data-num="${a.id}" type="checkbox"
									value="" /></td>
								<td>${a.name}</td>
								<c:if test="${a.deliveryReceitp==1}">
									<td>水和废水样品交接联单</td>
								</c:if>
								<c:if test="${a.deliveryReceitp==2}">
									<td>空气和废气样品交接联单</td>
								</c:if>
								<c:if test="${a.deliveryReceitp==3}">
									<td>土壤样品交接联单</td>
								</c:if>
								<c:if test="${a.inspectionSheet==1}">
									<td>水质样品送检单</td>
								</c:if>
								<c:if test="${a.inspectionSheet==2}">
									<td>环境空气和无组织废气样品送检单</td>
								</c:if>
								<c:if test="${a.inspectionSheet==3}">
									<td>土壤、底质样品送检单</td>
								</c:if>
								<c:if test="${a.inspectionSheet==4}">
									<td>固定污染源有组织废气样品送检单</td>
								</c:if>
								<td>${a.other}</td>
								<td><a href="javascript:;"
									class="tablelink changeAnalysisCatrgory" data-num="${a.id}">修改</a><a
									href="javascript:;" class="tablelink delAnalysisCategory"
