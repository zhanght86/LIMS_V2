...
<script src="js/module/reportIssued.js" type="text/javascript"
	charset="utf-8"></script>
...
<body>
	...
	<div class="formbody">
		<div class="itab">
			<ul>
				<li><a href="#tab1" class="selected">报告签发审核</a></li>
			</ul>
		</div>
			...
			<table class="tablelist" id="reportTable">
				...
				<tbody>
					<s:iterator value="#p.reportSet" id="r" status="L">
						<c:set var="theString" value="${typeStr }"/>
						<c:if test="${r.result >= 3 && fn:contains(theString,r.type) }">
							<tr>
								...
								<td style="text-align: center;">${r.reportAudit.firstUser.name }</td>
								<td style="text-align: center;">${r.reportAudit.firstDate }</td>
								<td style="text-align: center;">${r.reportAudit.secondUser.name }</td>
								<td style="text-align: center;">${r.reportAudit.secondDate }</td>
								...
								<td style="text-align: center;">
									<a href="javascript:;" class="tablelink viewReportAudit"
													data-id="${r.id }">查看</a>
								</td>
								<td style="text-align: center;">
									<c:if test="${r.result == 3}">
										<a href="javascript:;"
										class="tablelink auditReport" data-id="${r.id }"
										data-projectId="${p.id }">审核</a>
									</c:if>
									<c:if test="${r.result > 3}">
										<label>审核通过</label>
									</c:if>
