...
<body class="common">
					...
					<tr>
						...
						<td style="text-align: center;"><label
							for="package_good${dr.id }"><input type="radio"
								...
								id="package_good${dr.id }"
								${dr.package_condition=="0"?"checked":"" } />完好</label> <label
							for="package_tag${dr.id }"><input type="radio"
								name="package_condition_${dr.id }" value="1"
								id="package_tag${dr.id }"
								${dr.package_condition=="1"?"checked":"" } />破损</label> <label
							for="package_solid${dr.id }"><input type="radio"
								name="package_condition_${dr.id }" value="2"
								id="package_solid${dr.id }"
								${dr.package_condition=="2"?"checked":"" } />玷污</label></td>
	...
	<div class="formbody">
		...
		<ul class="forminfo">
			<li><label>采样日期</label><label class="viewLabel">${nowDate
					}</label></li>
								...
								.confirm(
										...
										function(action) {
											if (action == 'ok') {
												$
														.ajax({
															url : "sampleTransfer_sampleTransfer_receiveNonItem.action",
															data : {
																'viewId' : id,
																'desp' : desp,
																'saveWay' : saveWay
								...
								.confirm(
										...
										function(action) {
											if (action == 'ok') {
												$
														.ajax({
															url : "sampleTransfer_sampleTransfer_receiveBackNonItem.action",
															data : {
																'viewId' : id,
															},
															async : false,
			...
			.on(
					...
					function() {
						...
						var tag = $(this).parent().siblings('td:eq(4)').find(
								'input[type=text]').val();
						var additives = $(this).parent().siblings('td:eq(5)')
