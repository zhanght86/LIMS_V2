					...
					<c:if test="${not empty r.sampleInfo}">
						<tr>
							...
							<td style="text-align: center; text-indent: 0px;"><input
								type="number" class="dfinput" name="analysisResult"
								placeholder="请输入分析结果" value="${r.analysisResult }"
								style="width: 100%"></td>
							<td style="text-align: center;">
								<%-- <a href="javascript:;" class="tablelink saveItem"
									data-id="${r.id }">保存</a> --%> <c:if
		...
		$('.saveItem')
				.on(
						...
						function() {
							var me=this;
							var id = $(this).data("id");
							var result = $(this).parent('td').prev('td').find(
									"input[name=analysisResult]").val();
							$
									.ajax({
										url : "detectionResult_detectionResult_writeWater.action",
										data : {
											"viewId" : id,
