...
<%
		...
		if(entity.getGainSample().equals("1")){
			for(InspectionSheet_Result result : results){
				...
				for(AnalysisProject analysis:projects){
					...
					table.openCellRC(2 + i, 3).setValue(solid.getAnalysisResult()==0.0?"":solid.getAnalysisResult()+"");
					table.openCellRC(2 + i, 4).setValue(solid.getStandardVolume()==0.0?"":solid.getStandardVolume()+"");
					table.openCellRC(2 + i, 5).setValue(solid.getStandardDryFlow()==0.0?"":solid.getStandardDryFlow()+ "");
					table.openCellRC(2 + i, 6).setValue(solid.getConcentration()==0.0?"":solid.getConcentration()+ "");
					table.openCellRC(2 + i, 7).setValue(solid.getDischarge()==0.0?"":solid.getDischarge()+ "");
				i++;
			}
		}else{
			for(InspectionSheet_Result result : results){
	...
	poCtrl.setCaption(entity.getContractId() + "固定污染源有组织废气样品送检单");
	poCtrl.setServerPage("poserver.do");
	poCtrl.setFileTitle(entity.getContractId() + "固定污染源有组织废气样品送检单");
	poCtrl.webOpen("doc/solidIspect.doc",
