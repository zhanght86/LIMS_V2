...
<%
	...
	for(InspectionSheet_Result result : results){
		...
		for(AnalysisProject analysis:projects){
			...
			table.openCellRC(2 + i, 3).setValue(air.getAnalysisResult()==0.0?"":air.getAnalysisResult()+"");
			table.openCellRC(2 + i, 4).setValue(air.getStandardVolume()==0.0?"":air.getStandardVolume()+"");
			table.openCellRC(2 + i, 5).setValue(air.getConcentration()==0.0?"":air.getConcentration()+"");
		i++;
	}
		}else{
	for(InspectionSheet_Result result : results){
	...
	poCtrl.setCaption(entity.getContractId() + "环境空气和无组织废气样品送检单");
	poCtrl.setServerPage("poserver.do");
	poCtrl.setFileTitle(entity.getContractId() + "环境空气和无组织废气样品送检单");
	poCtrl.webOpen("doc/airInspect.doc",
