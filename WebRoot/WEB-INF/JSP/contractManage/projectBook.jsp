...
<body>
	...
	<h2
		...
		编号：<label style="width:100px" id="addPageIdentify">${project.contractId
			}</label>
	</h2>
		...
		<table border="1" cellspacing="0" cellpadding="0" width="108%">
			<tr>
				...
				<td width="83%" colspan="5" valign="top"><c:if
						test="${contractType==0 }">
						<p align="center">${tContract.customerName}</p>
					</c:if> <c:if test="${contractType==1 }">
						<p align="center">${sContract.client}</p>
					</c:if></td>
			</tr>
			<tr>
				...
				<td width="83%" colspan="5" valign="top"><c:if
						test="${contractType==0 }">
						<p align="center">${tContract.projectName}</p>

					</c:if> <c:if test="${contractType==1 }">
						<p align="center">${sContract.name}</p>
			...
			<tr>
				...
				<td width="83%" colspan="5" valign="top"><input type='text'
					...
					value="${tContract.projectAim }" /></td>
			</tr>
			<tr>
				<td width="16%"><p align="center">地址</p></td>
				<td width="32%" colspan="2" valign="top"><c:if
						test="${contractType==0 }">
						<input type='text' name="address"
							style="width:100%; height:30px; background:#e9e3e3"
							 value="${tContract.customerAddress }"
							data-alert="地址" />
					</c:if> <c:if test="${contractType==1 }">
						<input type='text' name="address"
							style="width:100%; height:30px; background:#e9e3e3"
							 value="${sContract.clientAddress }"
							data-alert="地址" />
					</c:if></td>
				<td width="9%"><p align="center">邮编</p></td>
				<td width="42%" colspan="2" valign="top"><input type='text'
					name="code" style="width:100%; height:30px; background:#e9e3e3"
					 value="${tContract.customerCode}"
					data-alert="邮编" /></td>
			</tr>
			<tr>
				<td width="16%"><p align="center">联系人</p></td>
				<td width="32%" colspan="2" valign="top"><input type='text'
					name="contact" style="width:100%; height:30px; background:#e9e3e3"
					 value="${tContract.customerPeople }"
					data-alert="联系人" /></td>

				<td width="9%"><p align="center">电话</p></td>
				<td width="42%" colspan="2" valign="top"><c:if
						test="${contractType==0 }">
						<input type='text' name="tel"
							style="width:100%; height:30px; background:#e9e3e3"
							 value="${tContract.customerTel }"
							data-alert="电话" />
					</c:if> <c:if test="${contractType==1 }">
						<input type='text' name="tel"
							style="width:100%; height:30px; background:#e9e3e3"
							 value="${sContract.clientTel }"
							data-alert="电话" />
					</c:if></td>
			</tr>
			<c:if test="${contractType==0 }">
				<tr>
					...
					<td width="83%" colspan="5" valign="top">
							...
							<s:iterator value="#tContract.items" id="i">
								<tr>
				...
				<tr>
					...
					<c:if test="${tContract.projectWay==0 }">
						<td width="69%" colspan="4"><p>以我单位通过计量认证或实验室认可的方法进行检测</p></td>
					</c:if>
					<c:if test="${tContract.projectWay==1 }">
						<td width="69%" colspan="4"><p>客户指定的方法：${tContract.projectWayText
								}</p></td>
					</c:if>
			...
			<c:if test="${contractType==1 }">
				<tr>
					<td width="16%" rowspan="3"><p align="center">委托内容</p></td>
					<td width="83%" colspan="5" valign="top"><textarea
							name="context"
							style="width:100%; height:100px;background:#e9e3e3"
							class="required" data-alert="委托内容"></textarea></td>
				</tr>
				<tr>
					<td width="14%" rowspan="2"><p align="center">检测方法</p></td>
					<td width="69%" colspan="4"><p>
							<input type="radio" name="projectWay" value="0" checked="checked" />以我单位通过计量认证或实验室认可的方法进行检测
						</p></td>
				</tr>
				<tr>
					<td width="69%" colspan="4"><p>
			...
			<tr>
				...
				<td width="32%" colspan="2"><p align="center">当前用户</p></td>
				<td width="17%" colspan="2"><p align="center">日期</p></td>
				<td width="33%"><p align="center">${nowDate }</p></td>
			</tr>
			<tr>
			...
			<tr>
				<td width="100%" colspan="6" valign="top"><p>
						备注：
						<textarea name="other"
							style="width:95%; height:100px;background:#e9e3e3;margin: 10px"></textarea>
					</p></td>
			</tr>
		</table>
	</form>
